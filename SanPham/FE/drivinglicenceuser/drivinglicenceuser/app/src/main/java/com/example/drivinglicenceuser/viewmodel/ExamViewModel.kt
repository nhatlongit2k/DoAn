...
class ExamViewModel(application: Application): ViewModel() {
    ...
    fun getExamForUser(authToken: String) = liveData(Dispatchers.IO){
        emit(Resource.loading(null))
        try {
            emit(Resource.success(examRepository.getExamForUser(authToken)))
        }catch (e: Exception){
            emit(Resource.error(null, e.message ?: "Error"))
        }
    }

    fun getExamForUserWithTypeId(authToken: String, id: Long) = liveData(Dispatchers.IO){
        emit(Resource.loading(null))
        try {
            emit(Resource.success(examRepository.getExamForUserWithTypeId(authToken, id)))
        }catch (e: Exception){
            emit(Resource.error(null, e.message ?: "Error"))
        }
    }

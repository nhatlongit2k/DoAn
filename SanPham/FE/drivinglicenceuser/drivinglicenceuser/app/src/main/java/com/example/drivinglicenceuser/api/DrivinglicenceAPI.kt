...
interface DrivinglicenceAPI {
    ...
    @DELETE("api/types/{id}")
    suspend fun deleteTypeLicence(@Path("id") id:Long): Response<Unit>
    ...
    @GET("api/exams/getExamForUser")
    suspend fun getExamForUser(@Header("Authorization") authToken: String): List<Exam>

    @GET("api/exams/getExamForUserByTypeId/{id}")
    suspend fun getExamForUserWithTypeId(@Header("Authorization") authToken: String, @Path("id")id: Long): List<Exam>
    ...
    @PUT("api/admin/users/update-for-user")
    suspend fun updateForUser(@Body user: User): User
    ...
    suspend fun registerAccount(@Query("userName") userName: String, @Query("password") password: String,
                                     @Query("firstName") firstName: String, @Query("lastName") lastName: String,
                                     @Query("email") email: String, @Query("createBy") createBy: String,
                                     @Query("phoneNumber") phoneNumber: String, @Query("image") image: String): Response<ResultRegisterAccount>?
    ...
    @GET("api/questions/getAllQuestionWithAnswerInExam/{id}")
    suspend fun getAllQuestionWithAnswerInExam(@Path("id") id: Long): List<Question>
    ...
    //result-test
    @POST("api/result-tests/saveAllResult")
    suspend fun saveAllResult(@Body resultTest: ResultTest): ResultData

    @DELETE("api/result-tests/deleteResult/{id}")
    suspend fun deleteResultTest(@Path("id") id: Long): ResultData

    //result-question
    @GET("api/result-questions/getResultQuestionByResultTest/{id}")
    suspend fun getResultQuestionsByResultTestId(@Path("id") id: Long): List<ResultQuestion>
}